...
def obj(x):
    ...
    results = {"fitness": np.linalg.norm(x),
              "cost": 1}
    return results
...
if __name__ == "__main__":
    ...
    if isinstance(space, ConfigSpace.ConfigurationSpace):
        dimension = len(space.get_hyperparameters())
    else:
        dimension = len(space)

    optimizer = DE(cs=space, f=obj, dimensions=len(space), pop_size=10, max_age=np.inf,
                 mutation_factor=0.8, crossover_prob=0.9, strategy='rand1_bin',
                 budget=None, encoding=False, dim_map=None)
    
    optimizer.run(generations=100)
    print("Best configuration", optimizer.inc_config, "with score", optimizer.inc_score)